...
**Container utility rows**: In `render_engine.js`, `message_bus` and `database` children are excluded from Kahn layering and reinserted afterward. Buses normally become dedicated spine rows near the median layer of their connections; buses with 4+ total connections are marked `_cornerAnchor`, right-aligned after db rows, and sized to 3× their base width. Databases are placed in tighter rows beneath the deepest connected service, with direct parent→db vertical routing when column-aligned.

**Hybrid route scoring**: `routeEdge(e, idx)` builds route candidates and chooses with `chooseBestRoute`. Node crossings are the hard first priority. Candidate scores then weigh already-routed edge overlaps, already-routed edge crossings, bend count, and path length. This is intentionally not a hard “no edge crosses another edge” rule; it reduces stacked corridors without forcing huge perimeter detours.
